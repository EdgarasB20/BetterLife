...
class _StepsPageState extends State<StepsPage> {
  ...
  void initState() {
    super.initState();
    _initHealthPermissions();
  }

  Future<void> _initHealthPermissions() async {
    await Permission.activityRecognition.request();
    await _stepsService.requestPermissions();
  }

  Future<void> _openEditor({int initialGoal = 10000}) async {
    await showModalBottomSheet(
      ...
      builder: (_) => AddStepsSheet(
        ...
        onSave: (steps, goal) async {
          await _stepsService.setDayGoal(
            uid: _uid,
            day: _selectedDay,
            goal: goal,
          );
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Tikslas išsaugotas')),
            );
          }
        },
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Nustatyti tikslą'),
      ),
      body: StreamBuilder<List<StepEntry>>(
        ...
        builder: (context, monthSnapshot) {
          ...
          return StreamBuilder<StepEntry?>(
            stream: _stepsService.watchGoal(_uid, _selectedDay),
            builder: (context, goalSnapshot) {
              final goalEntry = goalSnapshot.data;
              final goal = goalEntry?.goal ?? 10000;

              return FutureBuilder<int>(
                future: _stepsService.readStepsForDay(_selectedDay),
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            FilledButton.icon(
                              onPressed: () => _openEditor(initialGoal: goal),
                              icon: const Icon(Icons.edit_rounded),
                              label: const Text('Redaguoti tikslą'),
                            ),
                          ],
                        ),
                      ),
